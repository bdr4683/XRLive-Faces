...
class BlendshapeCalculator():
    ...
    def _calculate_mouth_landmarks(self):
        ...
        upper_head = self._get_landmark(self.blend_shape_config.CanonicalPpoints.upper_head) # Decreases when smiling, for whatever reason.

        # TODO: abs(yada[1] - blabla[1]) IS NOT face roataion independent, for some reason. Find another way!
        ...
        # # mouth_center_nose_dist = abs(mouth_center[1] - nose_tip[1])
        # print(
        #     '{:.2f}'.format(round(mouth_center[0], 2)), '{:.2f}'.format(round(mouth_center[1], 2)), '{:.2f}'.format(round(mouth_center[2], 2))
        #     , " | " ,
        #     '{:.2f}'.format(round(nose_tip[0], 2)), '{:.2f}'.format(round(nose_tip[1], 2)), '{:.2f}'.format(round(nose_tip[2], 2))
        # )
        ...
        # print('{:.2f}'.format(round(upper_head[0], 2)), '{:.2f}'.format(round(upper_head[1], 2)), '{:.2f}'.format(round(upper_head[2], 2)))
        # print('{:.2f}'.format(round(upper_nose_dist, 2)), " | ", '{:.2f}'.format(round(jaw_nose_dist, 2)), '{:.2f}'.format(round(mouth_center_nose_dist, 2)))
        # print('{:.2f}'.format(round(head_height, 2)))
        # # head_height = abs(upper_head[1] - lowest_chin[1])
        # # upper_nose_dist = abs(upper_head[1] - nose_tip[1])
        # # jaw_nose_dist = abs(nose_tip[1] - lowest_chin[1])

        jaw_open_ratio = jaw_nose_dist / head_height
        jaw_magic_dist = math.dist(self._get_landmark(4), lowest_chin) # TODO: Will a landmark here allow smiling to not open jaw?
        ratioPlusL = jaw_nose_dist / jaw_magic_dist
        
        print('{:.2f}'.format(round(ratioPlusL, 2)), "=", '{:.2f}'.format(round(jaw_nose_dist, 2)), "/", '{:.2f}'.format(round(jaw_magic_dist, 2)))
        jaw_open_ratio = ratioPlusL
        
        # print("", ['{:.2f}'.format(round(p, 2)) for p in upper_head], "\n", 
        # ['{:.2f}'.format(round(p, 2)) for p in nose_tip], "\n", 
        # ['{:.2f}'.format(round(p, 2)) for p in lowest_chin])
        # self._live_link_face.set_blendshape(ARKitFace.MouthFrownRight, max(min(mouth_frown_right, 1), 0))
        
        # TODO: When smiling with mouth closed, this value is erroneously high.
        jaw_open = self._remap_blendshape(FaceBlendShape.JawOpen, jaw_open_ratio)
        # print('{:.2f}'.format(round(jaw_open_ratio, 2)))
        ...
        self._live_link_face.set_blendshape(
            FaceBlendShape.MouthClose, mouth_close) # TODO: When smiling with mouth closed, this value is erroneously low.
        ...
        self._live_link_face.set_blendshape(
            FaceBlendShape.MouthRollUpper, (1 - self._remap_blendshape(FaceBlendShape.MouthRollUpper, upper_lip_dist)) * 1.0000) # TODO: When smiling with mouth closed, this value is erroneously high.

        upper_lip_nose_dist = nose_tip[1] - uppest_lip[1]
        self._live_link_face.set_blendshape(
            FaceBlendShape.MouthShrugUpper, (1 - self._remap_blendshape(FaceBlendShape.MouthShrugUpper, upper_lip_nose_dist)) * 1.0000) # TODO: When smiling with mouth closed, this value is erroneously high.
    ...
    def _calculate_eye_landmarks(self):
        ...
        def irisPos(points, iris):
            ...
            dx = midPoint[0] - iris[0]
            dy = midPoint[1] - iris[1] - eye_width * 0.075 # eye center y is slightly above midpoint
        ...
        iris_left_x, iris_left_y = irisPos(self.blend_shape_config.CanonicalPpoints.eye_left, iris_left)
        iris_right = self._get_landmark(468, True)
        iris_right_x, iris_right_y = irisPos(self.blend_shape_config.CanonicalPpoints.eye_right, iris_right)
        iris_left_x *= -1
        # print("[", iris_left_x, ", ", iris_left_y, "] [", iris_right_x, ", ", iris_right_y, "]")
        self._live_link_face.set_blendshape(
